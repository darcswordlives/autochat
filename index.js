...
function startInitialTimer() {
    if (autochatTimerId) clearTimeout(autochatTimerId);
    ...
    if (minSec >= maxSec) {
        console.error(`[${extensionName}] Invalid range on startup. Disabling.`);
        $("#autochat-enabled").prop("checked", false); extension_settings[extensionName].enabled = false; saveSettingsDebounced(); return;
    }
...
function startAutochatTimer() {
    ...
    if (minSec >= maxSec) {
        ...
        console.error(`[${extensionName}] Invalid range. Disabling.`);
        $("#autochat-enabled").prop("checked", false); extension_settings[extensionName].enabled = false; saveSettingsDebounced(); return;
    }
...
function handleTimerEnd(finalMessage) {
    ...
    if (throttleEnabled && (now - lastSentTime < throttleTime)) {
        console.log(`[${extensionName}] Message throttled.`);
        toastr.warning("Autochat message was throttled to prevent spam.");
    } else if (finalMessage) {
        console.log(`[${extensionName}] Timer ended! Sending message:`, finalMessage);
    ...
    if (totalRepeats > 0 && currentRepeatCount >= totalRepeats) {
        console.log(`[${extensionName}] Repeat limit reached. Stopping.`);
...
function onMinSecondsChange(event) {
    ...
    if (!isNaN(newMin) && newMin > 0) {
        ...
        if (newMin >= currentMax) { const newMax = newMin + 1; extension_settings[extensionName].maxSeconds = newMax; maxInput.val(newMax); }
        saveSettingsDebounced();
    }
}
