...
function sendImpersonatedMessage() {
    try {
        // Find and click the impersonate button to generate the text
        const impersonateButton = $("#impersonate_button");
        if (impersonateButton.length > 0) {
            console.log(`[${extensionName}] Triggering impersonation to generate text...`);
            impersonateButton.trigger('click');
            
            // Wait a brief moment for the text to populate, then send it
            setTimeout(() => {
                const messageText = $("#send_textarea").val();
                if (messageText) {
                    console.log(`[${extensionName}] Text generated. Sending impersonated message:`, messageText);
                    $("#send_but").trigger('click');
                } else {
                    console.warn(`[${extensionName}] No text was generated by impersonation. Cannot send message.`);
                }
            }, 500); // 500ms delay to allow text to populate
        } else {
